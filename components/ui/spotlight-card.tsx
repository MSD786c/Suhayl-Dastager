...
const glowColorMap = {
  ...
  brand: { base: 25, spread: 50 } // Custom brand color based on your #e95d2c
};
...
const GlowCard: React.FC<GlowCardProps> = ({ 
  ...
  // Set CSS custom properties when the component mounts or when glowColor changes
  useEffect(() => {
    if (cardRef.current) {
      ...
      cardRef.current.style.setProperty('--border-size', 'calc(var(--border, 2) * 1px)');
      cardRef.current.style.setProperty('--spotlight-size', 'calc(var(--size, 150) * 1px)');
      cardRef.current.style.setProperty('--hue', 'calc(var(--base) + (var(--xp, 0) * var(--spread, 0)))');
    }
  }, [base, spread]);
  ...
  const getInlineStyles = () => {
    // Create a CSSProperties object with known properties first
